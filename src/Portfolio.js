...
const Layout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen text-white font-sans relative" style={{ backgroundImage: `url(${backgroundImageUrl})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
      ...
      <main className="backdrop-blur bg-black/70 min-h-screen pt-20 pb-10 px-4">{children}</main>
    </div>
  );
};
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col justify-center items-center text-center">
      ...
      <p className="max-w-2xl mb-8 text-lg">
        Java Full Stack Developer with 4+ years of experience building scalable, secure web apps using Java, Spring Boot, React, Angular, and AWS.
      </p>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <button onClick={() => navigate('/skills')} className="px-6 py-4 text-xl bg-gradient-to-r from-cyan-500 to-blue-500 rounded-lg hover:scale-105 transition">Skills</button>
        <button onClick={() => navigate('/experience')} className="px-6 py-4 text-xl bg-gradient-to-r from-pink-500 to-purple-500 rounded-lg hover:scale-105 transition">Experience</button>
        <button onClick={() => navigate('/contact')} className="px-6 py-4 text-xl bg-gradient-to-r from-yellow-400 to-orange-500 rounded-lg hover:scale-105 transition">Contact</button>
      </div>
    </motion.div>
  );
};
